...
void mejorpromedio(struct registroEstudiantes *estudiantes);
main(){
...
void mejorpromedio(struct registroEstudiantes *estudiantes){
	int i,j;
	float sumapromedio,promedio[es],mayorpromedio=0;
	for(i=0;i<es;i++){
		sumapromedio=0;
		for(j=0;j<n;j++){
			sumapromedio=sumapromedio+(estudiante[i].estudiantes.notas[j]);
		}
	promedio[i]=sumapromedio/n;
	}
	for(i=0;i<es;i++){
		if (mayorpromedio<=promedio[i]){
			mayorpromedio=promedio[i];
			}
	}
	for(i=0;i<es;i++){
		if (mayorpromedio==promedio[i]){
			printf("\n Estudiante de mejor promedio: %s",estudiante[i].estudiantes.nombre);
			printf("\n Promedio: %.2f",mayorpromedio);
		}
	}
}
