...
static size_t hash_if (size_t key, size_t max)
{
	uint8_t arr[4];
	uint32_t key_32 = (uint32_t) key;  //Convertimos de 64 a 32 bits
	arr[3] = key_32 & 0x000000FF;
	arr[2] = (key_32 & 0x0000FF00) >>8;
	arr[1] = (key_32 & 0x00FF0000) >>16;
	arr[0] = (key_32 & 0xFF000000) >>24;

	uint8_t a = arr[3]^arr[0];
	uint8_t b = arr[2]^arr[1];
...
int main()
{
	MapPtr dictionary = Map_New( 11, 0L, hash_if, coll_res );
